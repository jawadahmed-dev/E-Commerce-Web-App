...
export class ShopComponent implements OnInit {
  ...
  getProducts()
  {
    this.shopService.getProducts(this.productParams).subscribe(resp =>
      {
        this.productParams.pageIndex = resp.result.pageIndex;
        this.productParams.pageSize = resp.result.pageSize;
  ...
  getProductTypes()
  {
    this.shopService.getProductTypes().subscribe(resp =>
      {
        this.productTypes = [{id:0, name:'All'}, ...resp.result];
      },
      error =>
      {
        console.log(error);
  ...
  getProductBrands()
  {
    this.shopService.getProductBrands().subscribe(resp =>
      {
        this.productBrands = [{id:0, name:'All'}, ...resp.result];
      },
      error =>
      {
        console.log(error);
